...
if [ -z $IMAGE_NAME ]; then
    echo -e "IMAGE_NAME env is required\n"
    exit 9;
fi
...
AWS_ECR_IMAGE_URI="${AWS_ECR_URI}/${IMAGE_NAME}"
...
docker tag base-aws-php:${PHP_VERSION} ${AWS_ECR_IMAGE_URI}:${AWS_IMAGE_TAG}
...
# docker tag base-aws-php:latest ${AWS_ECR_IMAGE_URI}:latest
